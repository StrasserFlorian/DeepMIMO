...
class XmlGenerator:
    ...
    def __init__(
        ...
        if self.version >= MIN_V4_VERSION:
    ...
    def load_templates(self) -> None:
        ...
        if self.version >= MIN_V4_VERSION:
            ...
            geometry_city_path = geometry_city_path.replace(".xml", ".v4.xml")
            geometry_terrain_path = geometry_terrain_path.replace(".xml", ".v4.xml")
    ...
    def set_study_area(self) -> None:
        ...
        max_z = tmp.findall(".//MaxZ")[0]
        max_z[0].attrib["Value"] = FLOAT_STR % (self.setup.study_area.zmax)

        min_z = tmp.findall(".//MinZ")[0]
        min_z[0].attrib["Value"] = FLOAT_STR % (self.setup.study_area.zmin)

        x_val = tmp.findall(".//X")[0]
        x_val[0].attrib["Value"] = " ".join(
            [f"{i:.4g}" for i in self.setup.study_area.all_vertex[:, 0]],
        )

        y_val = tmp.findall(".//Y")[0]
        y_val[0].attrib["Value"] = " ".join(
    ...
    def set_ray_tracing_param(self) -> None:
        ...
        x[0].attrib["Value"] = f"{self.setup.ray_tracing_param.max_paths}"

        x = tmp.findall(".//RaySpacing")[0]
        x[0].attrib["Value"] = FLOAT_STR % (self.setup.ray_tracing_param.ray_spacing)

        x = tmp.findall(".//Reflections")[0]
        x[0].attrib["Value"] = f"{self.setup.ray_tracing_param.max_reflections}"

        x = tmp.findall(".//Transmissions")[0]
        x[0].attrib["Value"] = f"{self.setup.ray_tracing_param.max_transmissions}"

        x = tmp.findall(".//Diffractions")[0]
        x[0].attrib["Value"] = f"{self.setup.ray_tracing_param.max_diffractions}"
        ...
        x[0].attrib["Value"] = f"{self.setup.ray_tracing_param.ds_max_reflections}"

        x = tmp.findall(".//DiffuseScatteringTransmissions")[0]
        x[0].attrib["Value"] = f"{self.setup.ray_tracing_param.ds_max_transmissions}"

        x = tmp.findall(".//DiffuseScatteringDiffractions")[0]
        x[0].attrib["Value"] = f"{self.setup.ray_tracing_param.ds_max_diffractions}"
    ...
    def set_txrx(self) -> None:  # noqa: PLR0915
        ...
        for txrx in self.txrx.txrx[::-1]:
            ...
            output_id = new_txrx.findall(".//OutputID")[0]
            output_id[0].attrib["Value"] = f"{txrx.txrx_id}"
            short_description = new_txrx.findall(".//ShortDescription")[0]
            short_description[0].attrib["Value"] = txrx.txrx_name
    ...
    def update(self) -> None:
        ...
        # self.set_origin()  # NOTE: the lat/lon location of the origin is not used in ray tracing
