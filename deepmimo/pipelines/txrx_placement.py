...
def gen_tx_pos(rt_params: dict[str, Any]) -> np.ndarray:
    ...
    for bs_idx in range(num_bs):
        bs_cartesian = convert_Gps2RelativeCartesian(
            rt_params["bs_lats"][bs_idx],
...
def gen_plane_grid(  # noqa: PLR0913 - explicit params for clarity
    ...
    if normal.lower() == "x":
        return np.stack([fixed.flatten(), grid_coord1.flatten(), grid_coord2.flatten()], axis=-1)
    if normal.lower() == "y":
        return np.stack([grid_coord1.flatten(), fixed.flatten(), grid_coord2.flatten()], axis=-1)
    # 'z' is default
    return np.stack([grid_coord1.flatten(), grid_coord2.flatten(), fixed.flatten()], axis=-1)
